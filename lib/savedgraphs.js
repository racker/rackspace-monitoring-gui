...
SavedGraphs.prototype.put = function(req, res) {
    ...
    // rewrite all series because lazy
    data.series = series;
    ...
    this.SavedGraph.findOneAndUpdate({ tenantId: req.session.tenantId, _id: id }, data, function (err, graph) {
        log.info('PUT', {username: req.session.username, tenantId: req.session.tenantId, ip: req.ip, id: id, found: !!graph, error: err});
        if (err || !graph) {
          res.send(404);
        } else {
          log.info('Graph ' + id + ' Updated.');
          res.send(graph);
        }
    }.bind(this));
};
